...
async def get_executive_metrics(
    ...
    metrics = ExecutiveMetrics(
        ...
        greeting_rate=round((row.greeting_count or 0) / total * 100, 1) if total > 0 else None,
        product_offer_rate=round((row.offer_count or 0) / total * 100, 1) if total > 0 else None,
        resolution_rate=round((row.resolved_count or 0) / total * 100, 1) if total > 0 else None,
